...
internal class RaceResultScraper
{
    ...
    internal List<RaceResult> ScrapeRaceResults(int startYear, int endYear)
    {
        ...
        List<Circuit> circuits = JsonDeserializer.DeserializeCollection<Circuit>("circuits");
        ...
        List<string> requestedYearsLinks = GetLinks(links, startYear, endYear);

        foreach (var link in requestedYearsLinks)
        {
            ...
            if (!DateOnly.TryParseExact(scrapedDateString, "dd MMM yyyy",
    ...
    private List<string> GetLinks(List<string> links, int startYear, int endYear)
    {
        return links.Where(l =>
        {
            string yearString = l[41..45];
